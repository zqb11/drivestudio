...
class HMR2_4dhuman(PHALP):
    ...
    # 利用目标检测器检测得到当前帧每个行人预测框坐标、调整宽高比的预测框坐标、二值化掩码、置信度、类别、id、标注
    def get_detections(self, image, frame_name, t_, additional_data=None, measurments=None):
        ...
        ) =  super().get_detections(image, frame_name, t_, additional_data, measurments)# 利用目标检测器检测得到当前帧每个行人预测框坐标、二值化掩码、置信度、类别、id、标注

        # Pad bounding boxes 调整预测框宽高比
...
def run_4DHumans(
    ...
    for cam_id in camera_list:# 遍历每个视角, 保存该视角预测行人信息
        ...
        if not os.path.exists(cached_video_path):# 检查该视角原始视频是否存在，没有则制作
            ...
            for image_path in image_paths:
                ...
                out.write(frame)
            out.release()
        cfg.video.source = cached_video_path# 原始视频路径
        
        output_dir = os.path.join(temp_dir, 'phalp_output')# 预测行人信息路径
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        cfg.video.output_dir = output_dir# 预测行人视频路径

        phalp_tracker = HMR2_4dhuman(cfg)# 3D人体外观姿态追踪器
        phalp_tracker.track()# # 读取视频源，逐帧进行目标检测、人体重建、深度追踪（DeepSORT），然后将结果序列化到results/demo_{i}.pkl中，并保存该视角预测轨迹视频
        
        # reset global hydra
        GlobalHydra.instance().clear()
    
    # remove temporary or useless files 要删除的临时文件
    dirs_to_remove = [
    ...
    pred_tracks_allcam = {}
    for i, cam_id in enumerate(camera_list):# 加载所有视角预测行人轨迹
        if already_done:
    ...
    if save_temp and not already_done: 
        for i, cam_id in enumerate(camera_list):
            os.rename(
                os.path.join(temp_dir, 'phalp_output', 'results', f'demo_{i}.pkl'),
                os.path.join(temp_dir, 'phalp_output', f'cam_{cam_id}.pkl')
            )# 将所有视角预测行人轨迹从results/demo_{i}.pkl移动到cam_{cam_id}.pkl
    os.system(f"rm -rf {os.path.join(temp_dir, 'phalp_output', 'results')}")
    
    return pred_tracks_allcam# 所有视角预测行人轨迹