...
# This will improve code reusability and make it easier to add support for new datasets
def project_human_boxes(
    ...
    for cam_id in camera_list:# 遍历相机视角
        ...
        collector = {}
        frames = []
        for frame_id, frame_ins_list in frame_infos.items():# 遍历每一帧
            ...
            if len(frame_ins_list) > 0:
                ...
                for instance_id in frame_ins_list:# 遍历这一帧的实例
                    ins = instances_meta[str(instance_id)]# 该实例信息
        ...
        if save_temp:# 将该视角所有时间帧的实例信息序列化
            # save collector to pkl
            json.dump(collector, open(pkl_path, "w"))
        
        collector_all[cam_id] = collector# 将该视角所有时间帧的实例信息加入collector_all
    
    return collector_all# 返回所有视角所有时间帧的实例信息